...
3.	Files might not have the same number of rows and these extra rows will need to be removed in order to match up the correct Feature IDs and taxonomy columns
a.	Sort each file by FeatureID
b.	Copy FeatureID column from taxonomy.tsv into a new column in feature-table.tsv
c.	Select both FeatureID columns and select Conditional Formatting > Highlight Cell Rules > Duplicate Values
d.	Remove any cells that are not selected as duplicates from a copy of the taxonomy file, so rows are not removed from original taxonomy file
...
8.	Copy all columns excluding the feature id column into a new excel file 
a.	This new sheet is the number of sequences per OTUs – name sheet as # sequences per OTU
9.	Scroll to the bottom of the sheet and make a new calculated row
a.	For each sample, calculate the sum of all OTUs
10.	Add a new sheet in the excel file and name it “Relativ. Abund”
a.	Copy the taxonomy information and the sample names into the new sheet
b.	This new sheet will calculate the relative abundance of each OTU
11.	In the first cell for the first sample/taxon use this calculation and modify as necessary
a.	=('# sequences per OTU'!H2/'# sequences per OTU'!$H$23055)*100 
...
14.	Create a new sheet called “OTUs by [variable]” to calculate average OTUs by whatever variable you want to compare
a.	Copy the same taxonomy columns into the new sheet
b.	Rather than listing each individual sample, create new column names based on variable groups
i.	Ex: treatment1-timepoint1, treatment1-timepoint2
15.	In the first cell for the first treatment, calculate the average relative abundance for each OUT using the corresponding samples from the “Relativ. Abund” sheet
a.	=AVERAGE(‘Relativ. Abund'!AY2,'Relativ. Abund !AP2,'Relativ. Abund’!BE2,'Relativ. Abund'!BJ2)
...
5.	Calculate the average number of each OTU using the same method as steps 12-15
a.	=AVERAGE(Raw_data!AY2,Raw_data!AP2,Raw_data!BE2,Raw_data!BJ2)
b.	Where AY2, AP2, BE2, and BJ2 are replicates of the same treatment/timepoint/etc.
...
8.	Each column will eventually be transposed into a row, and Excel can only have 16,384 columns, so in the newly pasted data, remove any data that would exceed that limit
a.	Ex) if there are 20,000 rows, delete data in rows 16,385-20,000
9.	Make a new sheet and name it “[FirstVariable]_[SecondVariable]”
a.	For example, if you want to compare treatments to the control, it would be “Control_Treatment1
...
11.	Copy the column with the average number for each OTUs for the first variable you want to look at and transpose it in the new sheet underneath the OTU number 
...
13.	The final layout should be: 
a.	Row 1: #OTU
b.	Row 2: Sample 1
c.	Row 3: Sample 2
14.	Copy this new sheet into a new Excel file and name the new file as “mydata”
15.	Open R (not RStudio) version 3.6.2
a.	Install and load packages apple, vegan, vegetarian, and Rcmdr
16.	Once R Commander opens in a new window, paste this code in the R Script box
a.	morisita<-M.homog(mydata, q=2, std=FALSE)
mean(morisita)
simmetric<-betadiver(mydata, method=22)
mean(simmetric)
17.	Data > Import Data > from Excel file
a.	Name of dataset: mydata
b.	OK > select the mydata file created in step 12 > OPEN
18.	Select the code entered in step 14 > Submit
19.	Results are in the output box
a.	Results range from 0 to 1
20.	How to interpret the results (species turnover):
a.	Values close to 1: Changes in dominant or rare taxa
